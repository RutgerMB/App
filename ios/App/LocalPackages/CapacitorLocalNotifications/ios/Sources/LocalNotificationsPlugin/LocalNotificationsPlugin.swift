...
public class LocalNotificationsPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    @objc func schedule(_ call: CAPPluginCall) {
        guard let notifications = lnJSObjectArray(call, "notifications") else {
            lnReject(call, "Must provide notifications array as notifications option")
            return
        }
        var ids = [String]()
        ...
        for notification in notifications {
            guard let identifier = notification["id"] as? Int else {
                lnReject(call, "Notification missing identifier")
                return
            }
            ...
            do {
                ...
                lnReject(call, "Unable to make notification: \(error.localizedDescription)")
                return
            }
            ...
            do {
                ...
                lnReject(call, "Unable to create notification, trigger failed: \(error.localizedDescription)")
                return
            }
            ...
            center.add(request) { (error: Error?) in
                if let theError = error {
                    ...
                    lnReject(call, theError.localizedDescription)
                }
            }
    ...
    @objc override public func requestPermissions(_ call: CAPPluginCall) {
        self.notificationDelegationHandler.requestPermissions { granted, error in
            guard error == nil else {
                lnReject(call, error!.localizedDescription)
                return
            }
    ...
    @objc func cancel(_ call: CAPPluginCall) {
        guard let notifications = lnJSObjectArray(call, "notifications"), notifications.count > 0 else {
            lnReject(call, "Must supply notifications to cancel")
            return
        }
    ...
    @objc func registerActionTypes(_ call: CAPPluginCall) {
        guard let types = lnJSObjectArray(call, "types") else {
            return
        }
    ...
    func handleScheduledNotification(_ call: CAPPluginCall, _ schedule: JSObject) throws -> UNNotificationTrigger? {
        ...
        if let at = at {
            ...
            if dateInfo.date! < Date() {
                lnReject(call, "Scheduled time must be *after* current time")
                return nil
            }
    ...
    @objc func removeDeliveredNotifications(_ call: CAPPluginCall) {
        guard let notifications = lnJSObjectArray(call, "notifications") else {
            lnReject(call, "Must supply notifications to remove")
            return
        }
