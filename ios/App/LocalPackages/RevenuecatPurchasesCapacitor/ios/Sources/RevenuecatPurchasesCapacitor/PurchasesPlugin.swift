...
public class PurchasesPlugin: CAPPlugin, PurchasesDelegate, CAPBridgedPlugin {
    ...
    private var customerInfoUpdateCalls: [String: CAPPluginCall] = [:]
    private var lastReceivedCustomerInfo: CustomerInfo?

    @objc func configure(_ call: CAPPluginCall) {
        guard let apiKey = call.getOrRejectString("apiKey") else { return }
        let appUserID = call.optionalString("appUserID")
        let storeKitVersionRaw = call.getString("storeKitVersion", "")
        var storeKitVersion = storeKitVersionRaw.isEmpty ? StoreKitVersion.default.name : storeKitVersionRaw
        let purchasesAreCompletedByString = call.optionalString("purchasesAreCompletedBy")
        ...
        if purchasesAreCompletedByString == PurchasesAreCompletedBy.revenueCat.name {
            ...
            if let purchasesAreCompletedByObject = call.optionalObject("purchasesAreCompletedBy") {
        ...
        let userDefaultsSuiteName = call.optionalString("userDefaultsSuiteName")
        let shouldShowInAppMessagesAutomatically = call.getBool("shouldShowInAppMessagesAutomatically", true)
        let entitlementVerificationMode = call.optionalString("entitlementVerificationMode")
        ...
        let preferredLocale = call.optionalString("preferredUILocaleOverride")
    ...
    @objc func redeemWebPurchase(_ call: CAPPluginCall) {
        ...
        guard let redemptionLink = webPurchaseRedemption["redemptionLink"] as? String else {
            rcReject(call, "WebPurchaseRedemption parameter did not have a redemptionLink key")
            return
        }
    ...
    @objc func addCustomerInfoUpdateListener(_ call: CAPPluginCall) {
        ...
        self.customerInfoUpdateCalls[call.callbackId] = call
    ...
    @objc func removeCustomerInfoUpdateListener(_ call: CAPPluginCall) {
        ...
        if let index = self.customerInfoUpdateListeners.firstIndex(of: callbackId) {
            ...
            self.customerInfoUpdateCalls[callbackId]?.keepAlive = false
            self.customerInfoUpdateCalls.removeValue(forKey: callbackId)
            wasRemoved = true
        }
        call.resolve([
    ...
    @objc func purchaseStoreProduct(_ call: CAPPluginCall) {
        ...
        guard let productId = storeProduct["identifier"] as? String else {
            rcReject(call, "Product parameter did not have identifier key")
            return
        }
    ...
    @objc func purchaseDiscountedProduct(_ call: CAPPluginCall) {
        ...
        guard let signedDiscountTimestamp = discount["timestamp"] as? Int else {
            rcReject(call, "Discount parameter did not have timestamp key")
            return
        }
        CommonFunctionality.purchase(product: productId,
    ...
    @objc func purchasePackage(_ call: CAPPluginCall) {
        ...
        guard let packageId = package["identifier"] as? String else {
            rcReject(call, "aPackage parameter did not have identifier key")
            return
        }
        guard let presentedOfferingContext = package["presentedOfferingContext"] as? [String: Any] else {
            rcReject(call, "aPackage parameter did not have presentedOfferingContext key")
            return
        }
    ...
    @objc func getStorefront(_ call: CAPPluginCall) {
        ...
        CommonFunctionality.getStorefront { storefront in
            if let storefront = storefront {
                ...
                rcReject(call, "Storefront info could not be obtained for account.")
            }
        }
    }
    ...
    @objc func getPromotionalOffer(_ call: CAPPluginCall) {
        ...
        guard let discountIdentifier = discount["identifier"] as? String else {
            rcReject(call, "Discount parameter did not have identifier key")
            return
        }
    ...
    @objc func getEligibleWinBackOffersForProduct(_ call: CAPPluginCall) {
        ...
        guard let product = call.getOrRejectObject("product"),
            ...
            rcReject(call, "Product does not contain an identifier.")
            return
        }
    ...
    @objc func getEligibleWinBackOffersForPackage(_ call: CAPPluginCall) {
        ...
        guard let aPackage = call.getOrRejectObject("aPackage"),
              ...
              let productID = product["identifier"] as? String else {
            rcReject(call, "Package did not contain a product with a product identifier.")
            return
        }
    ...
    @objc func purchaseProductWithWinBackOffer(_ call: CAPPluginCall) {
        ...
        guard let winBackOfferID = winBackOffer["identifier"] as? String else {
            rcReject(call, "Win-back offer does not contain an identifier.")
            return
        }
    ...
    @objc func purchasePackageWithWinBackOffer(_ call: CAPPluginCall) {
        ...
        guard let packageID = aPackage["identifier"] as? String else {
            rcReject(call, "Package does not contain an identifier.")
            return
        }
    ...
    @objc func setProxyURL(_ call: CAPPluginCall) {
        let url = call.optionalString("url")
    ...
    @objc func beginRefundRequestForEntitlement(_ call: CAPPluginCall) {
        ...
        guard let entitlementId = entitlementInfo["identifier"] as? String else {
            rcReject(call, "entitlementInfo parameter did not have identifier field")
            return
        }
    ...
    @objc func beginRefundRequestForProduct(_ call: CAPPluginCall) {
        ...
        guard let productId = storeProduct["identifier"] as? String else {
            rcReject(call, "storeProduct parameter did not have identifier field")
            return
        }
    ...
    @objc func showInAppMessages(_ call: CAPPluginCall) {
        guard self.rejectIfPurchasesNotConfigured(call) else { return }
        let intMessageTypes = call.optionalArray("messageTypes") as? [Int]
    ...
    @objc func overridePreferredUILocale(_ call: CAPPluginCall) {
        guard self.rejectIfPurchasesNotConfigured(call) else { return }
        let locale = call.optionalString("locale")
    ...
    public func purchases(_ purchases: Purchases, receivedUpdated customerInfo: CustomerInfo) {
        self.customerInfoUpdateListeners.forEach { callbackId in
            self.customerInfoUpdateCalls[callbackId]?.resolve(CommonFunctionality.encode(customerInfo: customerInfo))
        }
